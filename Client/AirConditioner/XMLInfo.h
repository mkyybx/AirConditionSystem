...
class XMLInfo
{
	...
	public:
		...
		string build_Login_ACK_doc(int isSucceed, string ID);
		...
		string build_Login_doc(Slave*,int);
		string build_N_Temp_Submit_Freq_doc(Slave*);
		Userinfo load_Login_doc(TiXmlElement*, Slave*);
		int load_Set_Temp_doc(TiXmlElement*,Slave*);
		Userinfo load_Login_ACK_doc(TiXmlElement*, Slave*);
		...
		void load_N_Wind_doc(TiXmlElement*, Slave*);
};
